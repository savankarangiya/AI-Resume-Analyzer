...
def prepend_original_contact_block(optimized_text, original_contact_block):
    ...
    # Remove duplicated candidate name heading generated by AI
    first_line = original_contact_block.split("\n")[0].strip()

    cleaned_text = re.sub(
        rf'\n?#\s*{re.escape(first_line)}\s*\n?',
        '\n',
        cleaned_text,
        flags=re.IGNORECASE
    )
    ...
    for url in extract_urls(original_contact_block):
        cleaned_text = re.sub(
            rf'.*{re.escape(url)}.*\n?',
            '',
            cleaned_text,
...
if uploaded_file is not None:
    ...
    if st.sidebar.button("🛠 Fix & Download Resume"):
        ...
        with st.spinner(""):
            ...
            - Do NOT use vague claims like "improved accuracy", "enhanced performance", "better results", "actionable insights", or "rigorous testing" unless exact evidence is present in the resume.
            - Do NOT write "achieving improved accuracy" unless the original resume contains a real accuracy score.
            - Do NOT claim collaboration, seamless collaboration, or team work unless the resume clearly mentions team work.
            - Keep project bullets factual and based only on original resume content.
            ...
            - Add practical impact only when the original resume clearly proves it.
